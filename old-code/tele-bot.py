...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.chat_id == source_group_1:
        ...
        await context.bot.send_message(chat_id=target_group_1, text=text_to_send, reply_to_message_id=reply_to_message_id_1)
    elif update.message.chat_id == source_group_2:
        text_to_send = update.message.text or "Tin nhắn không chứa văn bản."
        await context.bot.send_message(chat_id=target_group_2, text=text_to_send, reply_to_message_id=reply_to_message_id_2)

app = ApplicationBuilder().token("5893169055:AAEBvlKkeARb8MybRMZRBueVh9ukY8Ab_no").build()
