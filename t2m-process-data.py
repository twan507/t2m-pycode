def run_data():
    ...
    for index, df in eod_index_dict.items():
        index_price_chart_df = pd.concat([index_price_chart_df, df[[ 'date','stock','close']].iloc[:200]], axis=0)

    index_price_chart_df.columns = ['date', 'index_name', 'value']
    ...
    data = [
        ...
        {'name': 'market_sentiment', 'value': market_sentiment['sentiment'][market_sentiment.last_valid_index()]},
        {'name': 'liquidity', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Thị trường']['liquidity'].iloc[0]*100).round(2)}%"},
        {'name': 'vnindex_close', 'value': index_card_df[index_card_df['stock']=="VNINDEX"]['close'].item()},
        {'name': 'vnindex_change_percent', 'value': f"{(index_card_df[index_card_df['stock'] == 'VNINDEX']['change_percent'].item() * 100).round(2):+.2f}%"},
        {'name': 'vnindex_change_value', 'value': f"{index_card_df[index_card_df['stock'] == 'VNINDEX']['change_value'].item().round(2):+.2f}"},
        {'name': 'market_volume', 'value': f"{int(market_info_df['volume'].sum()):,} cổ phiếu"},
        {'name': 'market_value', 'value': f"{int(market_info_df['value'].sum()):,} tỷ"},
        {'name': '%_tang_gia', 'value':f"{int(market_info_df[market_info_df['name']=='Tăng giá']['count']/market_info_df['count'].sum().item()*100)}%"},
        {'name': '%_giam_gia', 'value':f"{int(market_info_df[market_info_df['name']=='Giảm giá']['count']/market_info_df['count'].sum().item()*100)}%"},
        {'name': '%_khong_doi', 'value':f"{int(market_info_df[market_info_df['name']=='Không đổi']['count']/market_info_df['count'].sum().item()*100)}%"},
        {'name': 'vn30f1m_close', 'value': index_card_df[index_card_df['stock']=="VN30F1M"]['close'].item()},
        {'name': 'vn30f1m_change_value', 'value': f"{index_card_df[index_card_df['stock'] == 'VN30F1M']['change_value'].item().round(2):+.2f}"},
        {'name': 'vn30f1m_volume', 'value': f"{index_card_df[index_card_df['stock']=='VN30F1M']['volume'].item():,} hợp đồng"},

        {'name': 'score_hsA', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất A']['score'].item():+.2f}"},
        {'name': 'liquidity_hsA', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất A']['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_hsA', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất A']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất A'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_hsA', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất A']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất A'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'top_group_hsA', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='A'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                                    và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='A'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"},

        {'name': 'score_hsB', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất B']['score'].item():+.2f}"},
        {'name': 'liquidity_hsB', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất B']['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_hsB', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất B']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất B'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_hsB', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất B']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất B'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'top_group_hsB', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='B'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                                    và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='B'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"},

        {'name': 'score_hsC', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất C']['score'].item():+.2f}"},
        {'name': 'liquidity_hsC', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất C']['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_hsC', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất C']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất C'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_hsC', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất C']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất C'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'top_group_hsC', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='C'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                                    và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='C'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"},

        {'name': 'score_hsD', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất D']['score'].item():+.2f}"},
        {'name': 'liquidity_hsD', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất D']['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_hsD', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất D']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất D'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_hsD', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất D']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất D'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'top_group_hsD', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='D'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                                    và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='D'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"},

        {'name': 'name_bot_cap', 'value': itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]},
        {'name': 'score_bot_cap', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['score'].item():+.2f}"},
        {'name': 'liquidity_bot_cap', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_bot_cap', 'value': f"{ceil((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['in_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_bot_cap', 'value': f"{floor((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['out_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},

        {'name': 'name_top_cap', 'value': itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]},
        {'name': 'score_top_cap', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['score'].item():+.2f}"},
        {'name': 'liquidity_top_cap', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['liquidity'].item()*100).round(2)}%"},
        {'name': 'inflow_top_cap', 'value': f"{ceil((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['in_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
        {'name': 'outflow_top_cap', 'value': f"{floor((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['out_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"},
