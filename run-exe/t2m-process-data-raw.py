...
for key in itd_industry_name.keys():
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['industry_name']==key]['stock'].tolist()
...
for key in itd_industry_perform.keys():
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['industry_perform']==key]['stock'].tolist()
    stock_list = list(set(temp_stock_list_full) & set(current_stock_list))
...
for key in itd_marketcap_group.keys():
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['marketcap_group']==key]['stock'].tolist()
    stock_list = list(set(temp_stock_list_full) & set(current_stock_list))
...
for nganh in itd_industry_name.keys():
    score_df = time_series.copy()
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['industry_name']==nganh]['stock'].tolist()
    stock_list = list(set(temp_stock_list_full) & set(current_stock_list))
...
for group in itd_industry_perform.keys():
    score_df = time_series.copy()
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['industry_perform']==group]['stock'].tolist()
    stock_list = list(set(temp_stock_list_full) & set(current_stock_list))
...
for marketcap in itd_marketcap_group.keys():
    score_df = time_series.copy()
    temp_stock_list_full = stock_classification_filtered[stock_classification_filtered['marketcap_group']==marketcap]['stock'].tolist()
    stock_list = list(set(temp_stock_list_full) & set(current_stock_list))
...
eod_group_liquidity_melted = eod_group_liquidity_df.iloc[:20].melt(id_vars=['date'], var_name='group_name', value_name='value')
eod_group_liquidity_melted = eod_group_liquidity_melted.rename(columns={'value':'liquidity'})
eod_group_score_melted = eod_group_score_df.iloc[:20, :-4].melt(id_vars=['date'], var_name='group_name', value_name='value')
eod_group_score_melted = eod_group_score_melted.rename(columns={'value':'score'})

eod_score_liquidity_melted = eod_group_liquidity_melted.merge(eod_group_score_melted, on=['date','group_name'], how='inner')

eod_score_liquidity_melted['group_name'] = eod_score_liquidity_melted['group_name'].map(name_map_dict)
...
itd_score_liquidity_df = itd_series.merge(itd_score_liquidity_df, on='date', how='left')
...
itd_group_liquidity_df = itd_series.merge(itd_group_liquidity_df, on='date', how='left')
itd_group_liquidity_melted = itd_group_liquidity_df.melt(id_vars=['date'], var_name='group_name', value_name='value')
itd_group_liquidity_melted = itd_group_liquidity_melted.rename(columns={'value':'liquidity'})

itd_group_score_df = itd_series.merge(itd_group_score_df, on='date', how='left')
itd_group_score_melted = itd_group_score_df.melt(id_vars=['date'], var_name='group_name', value_name='value')
itd_group_score_melted = itd_group_score_melted.rename(columns={'value':'score'})

itd_score_liquidity_melted = itd_group_liquidity_melted.merge(itd_group_score_melted, on=['date','group_name'], how='inner')

itd_score_liquidity_melted['group_name'] = itd_score_liquidity_melted['group_name'].map(name_map_dict)
...
itd_score_liquidity_last = pd.concat([eod_group_liquidity_df.iloc[0, 1:],eod_group_score_df.iloc[0,1:-4],eod_group_score_df.iloc[:5,1:-4].mean(axis=0),group_score_ranking.iloc[0].iloc[1:]], axis=1).reset_index()
itd_score_liquidity_last.columns = ['name','liquidity','score','score_t5','rank']
itd_score_liquidity_last['liquid_state'] = itd_score_liquidity_last['liquidity'].apply(lambda x: 'Rất thấp' if x < 0.5 else (
                                                                                                 'Thấp' if (x >= 0.5) & (x < 0.8) else (
                                                                                                 'Trung bình' if (x >= 0.8) & (x < 1.2) else (
                                                                                                  'Cao' if (x >= 1.2) & (x < 1.5) else 'Rất cao'))))

itd_score_liquidity_last['order'] = itd_score_liquidity_last['name'].map(order_map_dict)
itd_score_liquidity_last['group'] = itd_score_liquidity_last['name'].map(group_map_dict)
itd_score_liquidity_last['name'] = itd_score_liquidity_last['name'].map(name_map_dict)
...
temp_df1 = temp_df1.melt(id_vars=['date'], var_name='index_name', value_name='value')
...
for time_span, name in zip([20,60,120,240],['1M','3M','6M','1Y']):
    for index_name in temp_df1['index_name'].unique():
        temp_df2 = temp_df1.loc[temp_df1['index_name'] == index_name].iloc[:time_span]
...
market_top_10 = eod_score_df[['stock', 'industry_name','industry_perform','marketcap_group','close','price_change','t0_score','liquid_ratio']].sort_values('t0_score', ascending=False).iloc[:10]
market_top_10['type'] = 'top'

market_low_10 = eod_score_df[['stock', 'industry_name','industry_perform','marketcap_group','close','price_change','t0_score','liquid_ratio']].sort_values('t0_score', ascending=True).iloc[:10]
market_low_10['type'] = 'low'

market_top_stock = pd.concat([market_top_10, market_low_10], axis=0).reset_index(drop=True)
...
for name in group_stock_name_list:
    if name == 'all_stock':
        market_breath_list.append(calculate_breadth(name, stock_classification_filtered['stock'].tolist()))
...
temp_df = itd_score_liquidity_last[itd_score_liquidity_last['group'].isin(['A','B','C','D'])]
temp_df = temp_df.sort_values('score', ascending=False)['name'].reset_index(drop=True).reset_index()
industry_breath_df = market_breath_df[market_breath_df['group'].isin(['A','B','C','D'])].drop(columns=['group','order'])
...
itd_score_liquidity_last = itd_score_liquidity_last.merge(temp_df, on='name', how='left')
...
full_industry_ranking = eod_group_score_df[['date',                                     
        'ban_le', 'bao_hiem', 'bds', 'bds_kcn','chung_khoan', 
        'cong_nghe','cong_nghiep', 'dau_khi', 'det_may','dulich_dv',
        'dv_hatang', 'hoa_chat', 'htd', 'khoang_san', 'ngan_hang','tai_chinh',
        'thep', 'thuc_pham', 'thuy_san', 'van_tai', 'vlxd', 'xd','y_te']]
...
full_industry_ranking = pd.DataFrame(full_industry_ranking.iloc[0, 1:]).rename(columns={0: 'score'}).reset_index()
full_industry_ranking.columns = ['name', 'score']
full_industry_ranking['type'] = full_industry_ranking['score'].apply(lambda x: 'Tiền vào' if x >= 0 else 'Tiền ra')
...
temp_df1 = temp_df1.melt(id_vars=['date'], var_name='group_name', value_name='value')
...
for time_span, name in zip([20,60,120,240],['1M','3M','6M','1Y']):
    for index_name in temp_df1['group_name'].unique():
        temp_df2 = temp_df1.loc[temp_df1['group_name'] == index_name].iloc[:time_span]
        temp_df2['time_span'] = name
...
group_score_ranking_melted['group_name'] = group_score_ranking_melted['group_name'].map(name_map_dict)
...
for group in group_stock_list:
    ...
    elif group in stock_classification_filtered['industry_perform'].unique().tolist():
        temp_group_stock_list = stock_classification_filtered[stock_classification_filtered['industry_perform']==group]['stock'].tolist()
    elif group in stock_classification_filtered['marketcap_group'].unique().tolist():
    ...
    group_stock_df = eod_score_df[eod_score_df['stock'].isin(temp_group_stock_list)][['stock', 'industry_name','industry_perform','marketcap_group','close','price_change','t0_score','t5_score','liquid_ratio','rank']]
...
daily_report_df = daily_report_df.append({'name': 'report_datetime', 'value': update_time['date'].iloc[0][10:]}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'report_date', 'value': update_time['date'].iloc[0][10:20]}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'market_sentiment', 'value': market_sentiment['sentiment'][market_sentiment.last_valid_index()]}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Thị trường']['liquidity'].iloc[0]*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vnindex_close', 'value': index_card_df[index_card_df['stock']=="VNINDEX"]['close'].item()}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vnindex_change_percent', 'value': f"{(index_card_df[index_card_df['stock'] == 'VNINDEX']['change_percent'].item() * 100).round(2):+.2f}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vnindex_change_value', 'value': f"{index_card_df[index_card_df['stock'] == 'VNINDEX']['change_value'].item().round(2):+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'market_volume', 'value': f"{int(market_info_df['volume'].sum()):,} cổ phiếu"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'market_value', 'value': f"{int(market_info_df['value'].sum()):,} tỷ"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': '%_tang_gia', 'value':f"{int(market_info_df[market_info_df['name']=='Tăng giá']['count']/market_info_df['count'].sum().item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': '%_giam_gia', 'value':f"{int(market_info_df[market_info_df['name']=='Giảm giá']['count']/market_info_df['count'].sum().item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': '%_khong_doi', 'value':f"{int(market_info_df[market_info_df['name']=='Không đổi']['count']/market_info_df['count'].sum().item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vn30f1m_close', 'value': index_card_df[index_card_df['stock']=="VN30F1M"]['close'].item()}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vn30f1m_change_value', 'value': f"{index_card_df[index_card_df['stock'] == 'VN30F1M']['change_value'].item().round(2):+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'vn30f1m_volume', 'value': f"{index_card_df[index_card_df['stock']=='VN30F1M']['volume'].item():,} hợp đồng"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'score_hsA', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất A']['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_hsA', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất A']['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_hsA', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất A']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất A'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_hsA', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất A']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất A'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'top_group_hsA', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='A'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                            và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='A'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'score_hsB', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất B']['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_hsB', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất B']['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_hsB', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất B']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất B'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_hsB', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất B']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất B'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'top_group_hsB', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='B'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                            và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='B'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'score_hsC', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất C']['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_hsC', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất C']['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_hsC', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất C']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất C'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_hsC', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất C']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất C'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'top_group_hsC', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='C'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                            và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='C'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'score_hsD', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất D']['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_hsD', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']=='Hiệu suất D']['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_hsD', 'value': f"{ceil((market_breath_df[market_breath_df['name']=='Hiệu suất D']['in_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất D'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_hsD', 'value': f"{floor((market_breath_df[market_breath_df['name']=='Hiệu suất D']['out_flow'].item()/market_breath_df[market_breath_df['name']=='Hiệu suất D'][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'top_group_hsD', 'value': f"dòng tiền mạnh nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='D'].sort_values('score', ascending=False)['name'].iloc[:2].str.cat(sep=', ')} \
                                                                            và yếu nhất là {itd_score_liquidity_last[itd_score_liquidity_last['group']=='D'].sort_values('score')['name'].iloc[:2].str.cat(sep=', ')}"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'name_bot_cap', 'value': itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'score_bot_cap', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_bot_cap', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_bot_cap', 'value': f"{ceil((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['in_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_bot_cap', 'value': f"{floor((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]]['out_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[0]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)

daily_report_df = daily_report_df.append({'name': 'name_top_cap', 'value': itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'score_top_cap', 'value': f"{itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['score'].item():+.2f}"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'liquidity_top_cap', 'value': f"{(itd_score_liquidity_last[itd_score_liquidity_last['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['liquidity'].item()*100).round(2)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'inflow_top_cap', 'value': f"{ceil((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['in_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
daily_report_df = daily_report_df.append({'name': 'outflow_top_cap', 'value': f"{floor((market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]]['out_flow'].item()/market_breath_df[market_breath_df['name']==itd_score_liquidity_last[itd_score_liquidity_last['group']=='cap'].sort_values('score')['name'].iloc[-1]][['in_flow','out_flow']].sum(axis=1)).item()*100)}%"}, ignore_index=True)
...
daily_report_df = daily_report_df.append({'name': 'top5_stock', 'value': top5_stock}, ignore_index=True)
...
daily_report_df = daily_report_df.append({'name': 'bot5_stock', 'value': bot5_stock}, ignore_index=True)
...
save_dataframe_to_sql(eod_score_liquidity_melted,'eod_score_liquidity_melted', engine)
...
save_dataframe_to_sql(group_score_ranking_melted,'group_score_ranking_melted', engine)
save_dataframe_to_sql(itd_score_liquidity_melted,'itd_score_liquidity_melted', engine)
